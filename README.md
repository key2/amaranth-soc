...
`amaranth-soc` provides a complete SoC infrastructure for [Amaranth HDL](https://github.com/amaranth-lang/amaranth), combining the original upstream amaranth-soc foundations (CSR registers, memory maps, event handling, Wishbone bus, GPIO peripheral) with a full AXI4/AXI4-Lite bus infrastructure, cross-bus bridges, CSR integration, peripherals, and a bus-agnostic SoC builder.

## Core Components (from upstream amaranth-soc)

| Component | Description |
|-----------|-------------|
| **CSR registers** | `csr.Register`, `csr.Field`, `csr.Multiplexer`, `csr.Decoder` — control/status register infrastructure with field-level access control |
| **Memory maps** | `MemoryMap` — hierarchical address space management with windows and resources |
| **Event handling** | `event.Source`, `event.EventMap`, `event.Monitor` — interrupt event aggregation |
| **Wishbone bus** | `wishbone.Signature`, `wishbone.Interface`, `wishbone.Decoder`, `wishbone.Arbiter` — Wishbone B4 pipelined bus |
| **GPIO peripheral** | `gpio.Peripheral` — general-purpose I/O with CSR interface |

## Extended Components (from amaranth-soc-builder, now merged)

| Layer | Components |
|-------|-----------|
| **Bus definitions** | `AXI4LiteSignature`, `AXI4LiteInterface`, `AXI4Signature`, `AXI4Interface` |
| **Interconnect** | `AXI4LiteDecoder` (1→N), `AXI4LiteArbiter` (N→1), `AXI4LiteCrossbar` (N×M) |
| **Memory** | `AXI4LiteSRAM` — byte-strobed SRAM controller |
| **AXI4 full** | `AXIBurst2Beat` — burst address generator, `AXI4ToAXI4Lite` — burst decomposition adapter |
| **Bridges** | `WishboneToAXI4Lite`, `AXI4LiteToWishbone`, `AXI4LiteCSRBridge`, `BusAdapter` registry |
| **Peripherals** | `TimerPeripheral`, `InterruptController` |
| **SoC builder** | `SoCBuilder` / `SoC` — bus-agnostic configuration and elaboration |
| **Export** | `CHeaderGenerator` — C `#define` macros for memory maps and IRQs |
| **Simulation** | `axi_lite_write()`, `axi_lite_read()`, `wb_write()`, `wb_read()`, `wb_write_pipelined()`, `wb_read_pipelined()` — async testbench helpers |
| **Timeout** | `AXI4Timeout` — bus watchdog with SLVERR generation |
...
Simulation
├── axi_lite_write()                        (async AXI4-Lite write)
├── axi_lite_read()                         (async AXI4-Lite read)
├── wb_write()                              (async Wishbone write)
├── wb_read()                               (async Wishbone read)
├── wb_write_pipelined()                    (async Wishbone pipelined write)
└── wb_read_pipelined()                     (async Wishbone pipelined read)
```
...
- `data_width` (`32` or `64`) — Width of data signals (`wdata`, `rdata`). Per AXI4-Lite spec, only 32 and 64 are valid.
...
Defined in [`axi/decoder.py`](amaranth_soc/axi/decoder.py:13). Routes transactions from one master to N slaves based on address.
...
watchdog = AXI4Timeout(addr_width=16, data_width=32, timeout=256)
...
Defined in [`periph/intc.py`](amaranth_soc/periph/intc.py:29). Aggregates N IRQ sources into a single CPU interrupt.
...
**Constructor parameters:**
- `bus_standard` (`BusStandard`) — System bus standard. Supports `AXI4_LITE` and `WISHBONE`.
...
Defined in [`sim/axi.py`](amaranth_soc/sim/axi.py:1). Async testbench helpers for AXI4-Lite simulation.
...
|-----------|------:|---------|
| `test_axi_bus.py` | 41 | AXI enums, signatures, interfaces, memory maps |
| `test_bridges.py` | 45 | WB↔AXI bridges, AXI4→AXI4-Lite adapter |
| `test_axi_crossbar.py` | 24 | NxM crossbar construction and elaboration |
| `test_axi_timeout.py` | 23 | Timeout watchdog construction and elaboration |
| `test_axi_burst.py` | 21 | Burst-to-beat address generator |
| `test_bus_adapter.py` | 18 | Bridge registry, direct + two-hop chains |
| `test_axi_sram.py` | 15 | SRAM construction + simulation (write/read/strobes) |
| `test_axi_arbiter.py` | 11 | Arbiter construction and elaboration |
| `test_axi_decoder.py` | 10 | Decoder construction and elaboration |
| `test_axi_csr_bridge.py` | 7 | AXI4-Lite to CSR bridge |
| `test_sim_helpers.py` | 4 | Simulation helper imports and signatures |
| `test_wb_sim_integration.py` | — | Wishbone simulation helper integration tests |
| `test_soc_handlers.py` | — | SoC builder handler unit tests |

---

## Project Status

See [`STATUS.md`](STATUS.md) for a detailed breakdown of implemented vs. placeholder files.

### What's Implemented

- ✅ CSR registers, memory maps, event handling, Wishbone bus, GPIO peripheral (from upstream amaranth-soc)
- ✅ Full AXI4-Lite and AXI4 bus definitions with Amaranth `wiring.Signature`
- ✅ Address decoder (1→N), round-robin arbiter (N→1), NxM crossbar
...
- ✅ Timer peripheral and interrupt controller with CSR interfaces
- ✅ Bus-agnostic SoC builder (`SoCBuilder` / `SoC`)
- ✅ C header generator for memory maps and IRQs
...
- ✅ SoC builder handler architecture (`BusHandler`, `CSRHandler`, `IRQHandler`)

### What's Planned (Not Yet Implemented)

- ⏳ CPU wrappers (VexRiscv, etc.) — blocked on Verilog RTL cores
- ⏳ Communication peripherals (UART, SPI, I2C) — separate project phase
- ⏳ Export formats: CMSIS-SVD, linker scripts, device tree
- ⏳ Platform integration (`soc/platform.py`)
